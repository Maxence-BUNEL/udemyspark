...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val flipped = totalFriendsByCharacter.map( x => (x._2, x._1) ).sortByKey(false)
    
    // Find the max # of connections
    val mostPopular = flipped.max()
    val tenMostPopular = flipped.take(10).map(x => (x._1, x._2,namesRdd.lookup(x._2)(0)))
    ...
    tenMostPopular.foreach(println)
  }
